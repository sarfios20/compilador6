...
float array_variables[32][3];
int siguienteVariableDisponible = 0;

int buscarRegistroLibreT() {
    int i = 0;
    for (size_t i = 0; i <= 9; i++) {
        if (array_registros_t[i] == true) {
            array_registros_t[i] = false;
            return i;
        }
    }
    return -1;
...
struct nodo *new_leaf_num(double value) {
    ...
    array_variables[a->registro][2] = 1;
    return a;
}
