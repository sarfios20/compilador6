...
int array_variables_int[32][3];
int siguienteVariableDisponible_int = 0;
bool array_registros_i[10] = {true, true, true, true, true, true, true, true, true, true}; // For integer registers
...
// Nodo HOJA for integers
struct nodo *new_leaf_int(int value)
{
  ...
  a->nodetype = 'I'; // I for Integer Leaf
  a->l = NULL;
  a->r = NULL;
  a->intval = value;

  // Assign a register for integer
  a->registro = buscarRegistroLibreI(); // you will need a similar function to find a free integer register

  // Save the variable name in .data
  a->variableNum = siguienteVariableDisponible;
  siguienteVariableDisponible++; // Increment it

  printf("La variable%d almacena el valor %d en el registro $s%d \n", a->variableNum, a->intval, a->registro);

  // Register a new variable in .data with the value
  // It saves in the same position that register uses: For $s14 uses position 14
  array_variables[a->registro][0] = a->intval;
  array_variables[a->registro][1] = a->variableNum;
  array_variables[a->registro][2] = 1; // occupied
...
// Node leaf for boolean
struct nodo *new_leaf_boolean(int value, int registro)
{
  struct nodo *a = (struct nodo *)malloc(sizeof(struct nodo)); // Allocate memory for a new node
  if (!a)
  {
    exit(0); // If the new node is NULL, it means there is not enough memory
  }
  a->nodetype = 'BL'; // BL stands for Boolean
  a->l = NULL;
  a->r = NULL;
  a->value = value; 

  a->registro = registro; // Assigning register from the input parameter

  printf("The boolean value %d is stored in the register $f%d \n", a->value, a->registro);

  // Register a new variable in .data with the value
  // Store in the same position as the register it uses: For $f14, use position 14
  array_variables[a->registro][0] = a->value;
  array_variables[a->registro][1] = a->variableNum;
  array_variables[a->registro][2] = 1; // occupied
...
struct nodo* new_leaf_text(char* texto, int registro) {
  ...
  a->string = (char *)malloc((strlen(texto) + 1) * sizeof(char));
  if (!a->string) {
    free(a); // Free the memory allocated for the node
    exit(0); // Memory allocation failed
  }
  strcpy(a->string, texto); // Copy the string

  a->registro = registro; // Assign the registro field
...
double eval(struct nodo *a)
{
  ...
  case 'L':
    ...
    break;
  // NODO HOJA that is an integer
  case 'I': 
    v = a->intval; // get the integer value
    printf("-> HOJA donde se asigna %d (variable%d) al registro $s%d\n", v, a->variableNum, a->registro);
    // Cargar el valor en su registro asignado de tipo F
    fprintf(yyout, "  li $s%d, variable%d\n", a->registro, a->variableNum); // load immediate, li, for loading an integer
